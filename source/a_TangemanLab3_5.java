...
public class a_TangemanLab3_5 extends PApplet {
...
//==COLORS ====================
  ...
  int RbCol = color(0,0,255);
  int LbCol = color(0,0,255);;
  int select1;
  int select2;
  int select3;
  int stateSelect;
...
//=BUTTONS & LABELS======================
  ...
  try{
    keyPixel1=PApplet.parseInt(green(gui_key.pixels[mouseX+mouseY*gui_key.width])+4);
  }catch(Exception e){
  }
...
//===TITLE - YEAR======================================
  ...
  text("Year:  "+table.getString(yearConstrain,table.getColumnTitle(0)),width*.20f,450);
  
  if(stateIndex()!=-999){
    textSize(15);
    fill(117, 117, 110,fade*20);
    stateString=table.getString(stateIndex()-1,table.getColumnTitle(3));
    text(stateString,width*.27f,27);
    fadeout=100;
  }else {
    textSize(15);
...
public void mousePressed(){
  ...
  if(mouseX < 490 && mouseX > 477 
      && mouseY < 485 && mouseY > 462) {
    RbCol=color(255,0,0);
    barPos=PApplet.parseInt(hs1.newspos+=11);
...
public void mouseReleased(){
  scrollPos = map(PApplet.parseInt(hs1.getPos()),20,PApplet.parseInt(20+hs1.swidth),0,42);
  if (scrollInit){
    if(scrollPos<=1){
      year=0;
    }
    if (scrollPos!=1){
      year=51*PApplet.parseInt(scrollPos);
      generateValues();
    }
  }
      mouseCheck=false;
...
public void addButtons(String name,int recX, int recY, String onoff,int col,String smallLarge){
    textSize(25);
...
class Bubble extends Thread{
  ...
  Bubble(float x_, float y_, float r_, int c_, int indx_,int V1_,int V2_,int V3_) {
    x = x_;
    y = y_;
    r = r_;
    c = c_;
  ...
  public void displayLabel() {
    ...
    if (check1){
        if (mouseCheck && mouseX >550 && mouseX<950 && mouseY >60 && mouseY<450){
           check1=true;
           x = origX+((mouseX-origX)*.4f);
           y = origY+((mouseY-origY)*.4f);
        }else {
          x = origX;
          y = origY;
          check1=false; 
        }
      String txt = table.getString(indx,table.getColumnTitle(3)) + ", x: " + PApplet.parseInt(V1) + ", y: " + PApplet.parseInt(V2)+", r: "+PApplet.parseInt(V3);
...
public int stateIndex(){ //Check Pixel Color to Obtain Value
  if (mouseX >= 0 && mouseX <= 650
    ...
    //println(str(r)+" "+str(g)+" "+str(b)); //RGB Check
     ...
     else if ((r/5) < 52 || g!=255.0f){
       return PApplet.parseInt((PApplet.parseInt(r)/5));
     }else {
     return -999;
     }
   }else {
...
public boolean overRect(int recX, int recY, int width, int height) {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "a_TangemanLab3_5" };
    if (passedArgs != null) {
